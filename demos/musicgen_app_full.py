...
def _do_predictions(texts, melodies, duration, progress=False, gradio_progress=None, **gen_kwargs):
    ...
    try:
        # run melody-and-text-conditional generation
        if any(m is not None for m in processed_melodies) and any(t != "" for t in texts):
            logger.info(f"Running  melody-and-text-conditioned generation")
